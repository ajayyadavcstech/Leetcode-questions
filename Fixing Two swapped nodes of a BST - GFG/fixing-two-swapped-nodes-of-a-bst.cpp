...
class Solution {
  public:
    void Inorder(struct Node* root,vector<pair<int,struct Node*>>&v){
        if(!root) return ;
        Inorder(root->left,v);
    ...
    struct Node *correctBST(struct Node *root) {
        // code here
        vector<pair<int,struct Node*>> v;
        vector<pair<int,struct Node*>> v1;
        Inorder(root,v);
        v1=v;
        sort(v.begin(),v.end());
        for(int i=0;i<v.size();i++){
            if(v[i].first!=v1[i].first){
                swap(v[i].second->data,v1[i].second->data);
                break;
            }
        }
        return root;
